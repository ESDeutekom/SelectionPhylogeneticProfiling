...
## Adjusted from old code in project 2
#######################################################
...
IDtranslation_file = sys.argv[3] #translation file between ensembl id's used in biogrid and our set, downloaded from biomart
all_out_interactions = sys.argv[4] #interactions out file in translated IDs
...
def enz_to_ens(tax_ID, translation_file):
    ...
    for lines in translation_file:
        ...
        #if str(tax_ID) == str(tax_ID):
        if len(line) > 1: #if there is a translation
            Entz = line[1] #GeneID == EtrezGene
            ENSG = line[0] # Ensembl ID
...
def interaction_dict(tax_ID, BIOGRID_file):
    ...
    print("interactions: ", len(inter_dict))
    return inter_dict
...
print("Non redundant interactions done, length: ", len(nonR_dict))
